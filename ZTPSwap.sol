...
library SwapUtils {
    ...
    event NewFeeCollector(address newFeeCollector);
    ...
    struct Swap {
        ...
        // The address to collect the admin fee
     ...
     * @param newFeeCollector new address as fee collector
     */
    function setFeeCollector(Swap storage self, address newFeeCollector) external {
        require(newFeeCollector != address(0), "Address zero is not allowed");
        self.feeCollector = newFeeCollector;

        emit NewFeeCollector(newFeeCollector);
    }
}
 ...
 * deployment size.
 */
contract ZTPSwap is OwnerPausableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    /**
     * @notice Modifier to check if the sender is the fee collector
     */
    modifier onlyFeeCollector() {
        require(swapStorage.feeCollector != address(0), "Fee collector is not set. Call setFeeCollector to set it first");
        require(msg.sender == swapStorage.feeCollector, "Fee collector only!");
        _;
    }
    ...
    /**
     * @notice Withdraw all admin fees to the fee collector address.
     */
    function withdrawAdminFees() external onlyFeeCollector {
        swapStorage.withdrawAdminFees(swapStorage.feeCollector);
    }
    ...
    function setFeeCollector(address newFeeCollector) external onlyOwner {
        swapStorage.setFeeCollector(newFeeCollector);
    }
    ...
    function setLPToken(LPToken _lpToken) external onlyOwner {
